...
- Якщо повідомлення містить назву задачі ТА час (навіть якщо вони дублюються або склеєні), ОБОВ'ЯЗКОВО повертай intent: "create".
- Повертай intent: "chat" ТІЛЬКИ якщо в повідомленні ВЗАГАЛІ немає суті задачі (що зробити), і тоді використовуй код "WAIT_TIME:назва".
